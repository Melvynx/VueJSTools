  ...
  methods: {
    ...
    startRandomWord() {
      if (this.words.length > 1 && this.words.length < 1000) {
        this.randomizRandomWord();
        this.isAddingWord = false;
      } else {
        this.textInfoErreur =
          "On ne peut pas randomizer une seul et unique valeur !(ou rien :p)";
        this.infoErreur = true;
        setTimeout(() => {
          this.infoErreur = false;
        }, 5000);
    ...
    randomizRandomWord() {
      if (this.settingsOnlyOne) {
        randomArray = randomizArray(this.words);
        this.wordsRandoms = [];
        this.wordsRandoms.push(randomArray[0]);
      } else {
        randomArray = randomizArray(this.words);
  ...
  <transition name="fade">
    <div class="blockListWord" v-show="!isAddingWord">
      ...
      <div class="blockButtonListWord">
        <button v-on:click="turnOffListMode" class="buttonBackToAddWord">
          Modifier/Ajouter
        </button>
        <button
          v-on:click="randomizAgain"
          v-bind:class="{ transitionDown : transisRandomizerAgain }"
          class="buttonListWord"
        >
          Randomiz again
        </button>
      </div>
      <div class="blockButtonOneWord">
        <div class="toggle">
          <input type="checkbox" v-model="settingsOnlyOne" class="check" />
          <span class="b switch"></span>
          <span class="b track"></span>
        </div>
        <p class="ParaToggleButton">
